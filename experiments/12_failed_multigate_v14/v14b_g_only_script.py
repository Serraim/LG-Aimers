"""V14b(G-only) submission inference: 2단 게이트(G)만, R3(배합비 변경) 제외.

배경: V13(Z1 평탄게이트)은 실제 LB 1095.23으로 잘 전이됐다(V8 대비 +5.28,
내부예측 +3.3~3.7보다도 좋음). 그런데 V13 위에 G(2단게이트)와 R3(S61 배분
0.50->0.70)를 동시에 얹은 V14는 실제 LB 1090.89로 V13보다 오히려 -4.34
나빴다 -- 내부 OOF(2023/2024)로는 G도 R3도 개별로/합쳐서 전부 양수였는데
정반대로 뒤집혔다.

내부 재검증으로는 어느 쪽이 범인인지 가릴 수 없었다(둘 다 2023-R/2024-R에서
개별로도 통과). 그래서 실제 채점으로 분리한다 -- G만 넣은 버전이 이 스크립트다.

  G  게이트를 2스트라이크 여부로 나눈다 (V14와 동일, 변경 없음)
       R & 2스트라이크     w = 0.325
       R & 비2스트라이크   w = 0.100
  R3 없음. S61/S63 배분은 V13과 동일하게 0.50/0.50.

가설: 2025 KBO ABS 존이 2024 대비 상하단 0.6%p 하향됐다(투구판정존 상단
55.75%/하단 27.04%). 2스트라이크는 존 경계 유인구를 던지는 구간이라 존 변경에
가장 민감하다 -- G가 V14 역전의 범인일 가능성을 R3보다 높게 본다.
이 제출로 G 단독의 실제 전이율을 확인한다.

S37 메타 계수와 CENTER_SHIFT 는 그대로다.
"""
...
# V14b: R3(S61 배분 변경) 제외 -- V14 실제 채점이 V13보다 나빠서 G/R3 분리 검증.
# V13과 동일하게 0.50/0.50 유지.
S61_SHARE = 0.50
S63_SHARE = 0.50
