"""E61-C standalone submission inference; every test row is independent."""
...
def predict_control_success(test):
    ...
    if artifact.get("artifact_type") not in {
        "e61c_joint_seed_ensemble_v1",
        "e61b_single_joint_ensemble_v1",
    }:
        raise ValueError("E61-B/E61-C submission artifact is required")
    e42 = artifact["e42"]
    ...
    joint = artifact["joint_member"]

    wwb_matrix = build_member_features(test, binary, True)
    binary_prediction = binary["model"].predict_proba(wwb_matrix)[:, 1]
    joint_prediction = np.mean(
        np.column_stack(
            [
                joint_success_probability(model, wwb_matrix, joint["success_labels"])
    ...
    lgbm_prediction = (1.0 - alpha) * binary_prediction + alpha * joint_prediction
    ...
    catboost_prediction = catboost["model"].predict_proba(
        build_member_features(test, catboost, True)
    )[:, 1]
    xgboost_prediction = xgboost["model"].predict_proba(
    ...
    trackman_prediction = np.sum(expert_predictions * trackman_probabilities, axis=1)
    weights = e42["weights"]
    prediction = (
        weights["wwb_lgbm"] * lgbm_prediction
        + weights["season_residual"] * residual_prediction
        + weights["catboost"] * catboost_prediction
        + weights["xgboost"] * xgboost_prediction
        + weights["trackman_moe"] * trackman_prediction
    )
    # test 분포가 아니라 E61-C 2024 validation center error에서 고정한 shift다.
